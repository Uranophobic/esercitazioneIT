...
public class Ticket {private Connection con = null;

// non mettere la primary-key nell'inserimento
public void inserire(int idstud, int idammministrativo, String richiesta ) {
	boolean chiuso = false;
	String query = "INSERT INTO ticket(idstud, idammministrativo, richiesta, chiuso) values(?,?,?,?)";
	try {
		con = Connessione.getInstance().getConnection();
		PreparedStatement pst = con.prepareStatement(query);
		pst.setInt(1, idstud);
		pst.setInt(2, idammministrativo);
